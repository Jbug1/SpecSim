...
def weighted_dot_product_distance(
    spec_query, spec_reference, ms2_ppm=None, ms2_da=None
):
    r"""
...
def ms_for_id_v1_distance(spec_query, spec_reference, ms2_ppm=None, ms2_da=None):
    ...
    spec_matched = match_peaks_in_spectra(
        spec_a=spec_query, spec_b=spec_reference, ms2_ppm=ms2_ppm, ms2_da=ms2_da
    )
...
def ms_for_id_distance(spec_query, spec_reference, ms2_ppm=None, ms2_da=None):
    ...
    s2 = np.power(n_q + 2 * n_r, d) + np.sum(np.abs(i_delta)) + np.sum(np.abs(m_delta))
...
def ms_for_id_mod_3_distance(p, q):
    "max is 1"
    ...
    # calculate common peaks similarity and map to 0-1 interval
    dist = ms_for_id_distance(p, q)

    # multiply common peaks similarity by the matched peak mass as % of total
    return dist * 2 / (sum(p) + sum(q))


def ms_for_id_reverse_distance(p, q):

    p, q = select_common_peaks_2(p, q)
    if np.sum(p) == 0:
        score = 1
    else:
        score = ms_for_id_distance(p, q)

    return score


def max_ms_for_id_distance(p, q):

    forward = ms_for_id_reverse_distance(p, q)
    backward = ms_for_id_reverse_distance(q, p)

    return max(forward, backward)


def min_ms_for_id_distance(p, q):
